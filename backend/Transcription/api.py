...
# Define paths for word cloud images
...
def generate_word_cloud(words, output_path, title="Word Cloud"):
    ...
    """
    if not words:
        # Create an empty word cloud if no words are provided
...
def get_transcription_analytics(
    ...
    try:
        ...
        # Get transcription IDs for completed transcriptions
        transcription_ids = db.query(Transcription.id).filter(
            Transcription.audio_id.in_(recording_ids)
        ).all()
        transcription_ids = [t[0] for t in transcription_ids]
        
        # Get transcribe_ai data for completed transcriptions
        ...
        # Process positive and negative keywords for word clouds
        ...
        # Process category interest (derived from contact reasons)
        # Extract unique categories from contact reasons
        ...
        # Process audience demographics from feedback table
        # Get all feedback records for the filtered recordings
        ...
        # Determine threshold for "frequent" numbers (e.g., more than 1 occurrence)
        frequent_threshold = 1
        
        # Count frequent and new numbers
