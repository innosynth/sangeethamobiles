...
def upgrade() -> None:
    ...
    sa.Column('status', sa.Enum('ACTIVE', 'INACTIVE', 'PENDING', 'FAILURE', name='statusenum'), nullable=False),
    ...
    op.add_column('user', sa.Column('status', sa.Enum('ACTIVE', 'INACTIVE', 'PENDING', 'FAILURE', name='statusenum'), nullable=False))
...
def downgrade() -> None:
    ...
    op.add_column('user', sa.Column('user_role', mysql.ENUM('L4', 'L3', 'L2', 'L1', 'L0'), nullable=False))
    op.add_column('user', sa.Column('business_key', mysql.VARCHAR(length=255), nullable=False))
    op.add_column('user', sa.Column('user_status', mysql.ENUM('ACTIVE', 'INACTIVE', 'PENDING', 'FAILURE'), nullable=False))
    op.add_column('user', sa.Column('id', mysql.VARCHAR(length=36), nullable=False))
    op.add_column('user', sa.Column('store_id', mysql.VARCHAR(length=36), nullable=False))
    op.alter_column('user', 'modified_at',
               existing_type=mysql.DATETIME(),
               nullable=False)
    op.alter_column('user', 'created_at',
