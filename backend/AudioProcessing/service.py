...
def upload_recording(Recording, staff_id, start_time, end_time, CallDuration, db, token):
    ...
    with open(file_path, 'rb') as data:
            ...
            formatted_datetime = current_datetime.strftime("%Y%m%d%H%M%S%f")+"."+str(etn[0])
            upload = client_s3.upload_fileobj(data, 
                                            S3_bucket_name,
